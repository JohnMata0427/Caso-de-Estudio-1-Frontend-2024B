  ...
  template: `
    <admin-layout class="flex flex-col min-h-dvh">
      <aside class="flex gap-x-2 items-center font-medium text-sm">
        ...
        <h2>Detalles del Estudiante con ID {{ id() }}</h2>
      </aside>
      <article class="h-40">
      ...
      <aside class="flex gap-x-2 items-center font-medium text-sm">
        ...
        <h2>Matriculas del Estudiante:</h2>
      </aside>
      <table-component
        title="estudiantes"
        [loading]="estudianteResource.isLoading()"
...
export class EstudiantesByIdAdminPage {
  ...
  public readonly estudianteResource = rxResource({
    loader: () => this.estudiantesService.getById(this.id()),
  });
